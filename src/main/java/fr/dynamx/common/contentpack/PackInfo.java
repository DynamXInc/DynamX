        ...
        if (DynamXErrorManager.getErrorManager().getAllErrors().containsKey(getFixedPackName()) && !DynamXErrorManager.getErrorManager().getAllErrors().get(getFixedPackName()).getErrors().isEmpty()) {
            if (!StringUtils.isNullOrEmpty(compatibleLoaderVersions)) {
                try {
                    ...
                    if (!range.containsVersion(DynamXConstants.PACK_LOADER_VERSION)) {
                        ...
                        DynamXErrorManager.addError(getFixedPackName(), "pack_requirements", ErrorManagerService.ErrorLevel.LOW, "pack_version", "This pack is made for versions " + compatibleLoaderVersions + " of the DynamX's pack loader (currently in version " + DynamXConstants.PACK_LOADER_VERSION.getVersionString() + ")", null, 600);
                    }
            ...
            if (!StringUtils.isNullOrEmpty(dcFileVersion) && packType.isCompressed()) {
                if (!dcFileVersion.equalsIgnoreCase(DynamXConstants.DC_FILE_VERSION)) {
                    ...
                    DynamXErrorManager.addError(getFixedPackName(), "pack_requirements", ErrorManagerService.ErrorLevel.HIGH, "pack_dc_version", "The model files are compiled for version " + dcFileVersion + " of the DynamX's .dc file loader (currently in version " + DynamXConstants.DC_FILE_VERSION + "). The pack will take more time to load.", null, 600);
                }
            }
                ...
                if (!StringUtils.isNullOrEmpty(addonInfo.versions) && AddonLoader.isAddonLoaded(addonInfo.addonId)) {
                    try {
                        ...
                        if (!range.containsVersion(new DefaultArtifactVersion(AddonLoader.getAddons().get(addonInfo.addonId).getVersion()))) {
                            ...
                            DynamXErrorManager.addError(getFixedPackName(), "pack_requirements", ErrorManagerService.ErrorLevel.LOW, "pack_addon_dependencies", "This pack is made for versions " + addonInfo.versions + " of the addon " + addonInfo.addonId + " (currently in version " + AddonLoader.getAddons().get(addonInfo.addonId).getVersion() + ")", null, 600);
                        }
                    } catch (InvalidVersionSpecificationException ignored) {
                    } //Already caught in onComplete
                }
    ...
    public void onComplete(boolean hotReload) {
        ...
        if (!StringUtils.isNullOrEmpty(compatibleLoaderVersions)) {
            try { //Check format of the version specs
                ...
                DynamXErrorManager.addError(getFixedPackName(), "pack_requirements", ErrorManagerService.ErrorLevel.FATAL, "pack_version", "Bad CompatibleWithLoaderVersions property", e);
        ...
        for (RequiredAddonInfo addonInfo : required) {
            if (!AddonLoader.isAddonLoaded(addonInfo.addonId)) {
                ...
                DynamXErrorManager.addError(getFixedPackName(), "pack_requirements", ErrorManagerService.ErrorLevel.FATAL, "pack_addon_dependencies", "This pack requires the addon " + addonInfo.addonId + " in order to be loaded", null, 700);
            }
            if (!StringUtils.isNullOrEmpty(addonInfo.versions) && AddonLoader.isAddonLoaded(addonInfo.addonId)) {
                try { //Check format of the version specs
                    ...
                    DynamXErrorManager.addError(getFixedPackName(), "pack_requirements", ErrorManagerService.ErrorLevel.FATAL, "pack_addon_dependencies", "Bad required addon " + addonInfo.getFullName() + " Versions syntax in pack_info", e);
                    addonInfo.versions = "";
                }
            }
        }
