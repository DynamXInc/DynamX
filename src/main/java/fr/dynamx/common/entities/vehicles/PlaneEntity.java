...
import fr.dynamx.common.physics.entities.PlanePhysicsHandler;
...
public class PlaneEntity<T extends PlanePhysicsHandler<?>> extends BaseVehicleEntity<T> implements
    ...
    public PlaneEntity(String name, World world, Vector3f pos, float spawnRotationAngle, int metadata) {
        super(name, world, pos, spawnRotationAngle, metadata);
    }

    @Override
    public T createPhysicsHandler() {
        return (T) new PlanePhysicsHandler(this);
    }
    ...
    public ModularVehicleInfo createInfo(String infoName) {
        return DynamXObjectLoaders.PLANES.findInfo(infoName);
    }
